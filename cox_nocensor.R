...
# 情境 A：假設完全無設限 → 全部視為事件
train_outer$status <- 1L
test_outer$status  <- 1L
train_inner$status <- 1L
val_inner$status   <- 1L

# 建立 Surv 物件（後面用）
y_inner <- Surv(train_inner$rfstime)
y_val   <- Surv(val_inner$rfstime)
y_test  <- Surv(test_outer$rfstime)
...
c_index <- function(time, status, risk_score) {
  survival::concordance(Surv(time, status) ~ risk_score)$concordance
}
...
label_tr <- train_outer$rfstime
label_te <- test_outer$rfstime
...
simselex_cox <- function(dat, time_col, status_col,
  ...
  # --- (Fix) 讓 group-lasso 的 lambda 走到更小，避免整路徑全 0 ---
  # 1) 先用 grpreg 產生一條「更小 lambda.min」的路徑（nlambda 可加大）
  gpath <- grpreg::grpreg(
    X_big, y_vec,
    group = grp,
    penalty = "grLasso",
    family  = "gaussian",
    nlambda = 200,
    lambda.min = 1e-8,   # <<< 關鍵：比預設更小，讓懲罰更弱 :contentReference[oaicite:1]{index=1}
    eps = 1e-6
  )
  
  # 2) 把這條 lambda 路徑交給 CV（避免 cv 自己產生的 lambda 不夠小）
  #    並加上 0 當保底（lambda=0 等於不做正則化，至少不會全 0）
  lambda_seq <- c(gpath$lambda, 0)
  
  set.seed(seed)
  cvg <- grpreg::cv.grpreg(
    X_big, y_vec,
    group = grp,
    penalty = "grLasso",
    family  = "gaussian",
  ...
  # 3) 取 cv 的最佳 lambda
  lambda_star <- cvg$lambda.min
  
  # 4) 取係數：新版 grpreg 的 cv.grpreg 物件通常可直接 coef(cvg, ...) :contentReference[oaicite:2]{index=2}
  #    若你版本較舊，就 fallback 用 cvg$fit
  coef_vec <- tryCatch(
    as.numeric(coef(cvg, lambda = lambda_star)),
    error = function(e) as.numeric(coef(cvg$fit, lambda = lambda_star))
  )
  
  gamma_hat <- coef_vec[-1]  # drop intercept
  gamma_mat <- matrix(gamma_hat, nrow = q, ncol = p)
  
  # 5) 若還是全 0（selected 空），就用「最小 lambda」再取一次（保底）
  selected <- which(colSums(abs(gamma_mat)) > 0)
  if (length(selected) == 0) {
    lambda_star2 <- min(lambda_seq)
    coef_vec2 <- tryCatch(
      as.numeric(coef(cvg, lambda = lambda_star2)),
      error = function(e) as.numeric(coef(cvg$fit, lambda = lambda_star2))
    )
    gamma_hat <- coef_vec2[-1]
    gamma_mat <- matrix(gamma_hat, nrow = q, ncol = p)
    selected <- which(colSums(abs(gamma_mat)) > 0)
  }
...
cox_offset_inner <- coxph(Surv(rfstime) ~ offset(lp_train_inner), data = train_inner, x = TRUE)
...
cox_offset_outer <- coxph(Surv(rfstime) ~ offset(lp_train_outer), data = train_outer, x = TRUE)
...
c1 <- survival::concordance(Surv(test_outer$rfstime, test_outer$status) ~ rsf_risk)$concordance
c2 <- survival::concordance(Surv(test_outer$rfstime, test_outer$status) ~ I(-rsf_risk))$concordance
c(c1 = c1, c2 = c2, sum = c1 + c2)
